    ...
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                int hor = width - x - 1; int ver = height - y - 1;
                PlaceList eight = new PlaceList();
                for (int i = 1; i <= ver; i++) {
                    eight.add(i - 1, pl(x, y + i));
                }
                M[x][y][8] = eight;
                PlaceList one = new PlaceList();
                for (int i = 1; i <= min(hor, ver); i++) {
                    one.add(i - 1, pl(x + i, y + i));
                }
                M[x][y][1] = one;
                PlaceList two = new PlaceList();
                for (int i = 1; i <= hor; i++) {
                    two.add(i - 1, pl(x + i, y));
                }
                M[x][y][2] = two;
                PlaceList three = new PlaceList();
                for (int i = 1; i <= min(hor, y); i++) {
                    three.add(i - 1, pl(x + i, y - i));
                }
                M[x][y][3] = three;
                PlaceList four = new PlaceList();
                for (int i = 1; i <= y; i++) {
                    four.add(i - 1, pl(x, y - i));
                }
                M[x][y][4] = four;
                PlaceList five = new PlaceList();
                for (int i = 1; i <= min(x, y); i++) {
                    five.add(i - 1, pl(x - i, y - i));
                }
                M[x][y][5] = five;
                PlaceList six = new PlaceList();
                for (int i = 1; i <= x; i++) {
                    six.add(i - 1, pl(x - i, y));
                }
                M[x][y][6] = six;
                PlaceList seven = new PlaceList();
                for (int i = 1; i <= min(x, ver); i++) {
                    eight.add(i - 1, pl(x - i, y + i));
                }
                M[x][y][7] = seven;
                PlaceList zero = new PlaceList();
                zero.addAll(one); zero.addAll(two); zero.addAll(three);
                zero.addAll(four); zero.addAll(five); zero.addAll(six);
                zero.addAll(seven); zero.addAll(eight);
                M[x][y][0] = zero;
            }
        }
        return M;
