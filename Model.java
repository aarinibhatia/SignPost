...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        _solnNumToPlace = new Place[last + 1]; _solnNumToPlace[0] = null;
        for (int a = 0; a < _width; a++) {
            for (int b = 0; b < _height; b++) {
                int k = _solution[a][b];
                _solnNumToPlace[k] = pl(a, b);
            }
        }
        ...
        for (int a = 0; a < _width; a++) {
            for (int b = 0; b < _height; b++) {
                ...
                if (k >= 1 && k <= last) {
                    if (k == 1) {
                        _board[a][b] = new
                                Sq(a, b, 1, true, arrowDirection(a, b), 0);
                    } else if (k == last) {
                        _board[a][b] = new Sq(a, b, last, true, 0, 0);
                    } else if (k > 1 && k < last) {
                        _board[a][b] = new
                                Sq(a, b, 0, false, arrowDirection(a, b), -1);
                    }
                    _board[a][b]._successors = new PlaceList();
                    _board[a][b]._predecessors = new PlaceList();
                    _allSquares.add(_board[a][b]);
                }
            }
        }
        ...
        for (int k = 1; k <= last; k++) {
            if (_solnNumToPlace[k] == null) {
                throw badArgs("Not all sequence numbers present", "k");
            }
        }
        for (int a = 0; a < _width; a++) {
            for (int b = 0; b < _height; b++) {
                for (Place ss : _allSuccessors[a][b][arrowDirection(a, b)]) {
                    _board[a][b]._successors.add(ss);
                }
            }
        }
        ...
        for (int w = 0; w <= _width - 1; w++) {
            for (int h = 0; h <= _height - 1; h++) {
                for (Place p : _board[w][h]._successors) {
                    Sq sq2 = get(p);
                    sq2._predecessors.add(pl(w, h));
                }
    ...
    Model(Model model) {
        ...
        _allSuccessors = new PlaceList[_width][_height][9];
        ...
        for (int a = 0; a < _width; a++) {
            for (int b = 0; b < _height; b++) {
                _board[a][b] = new Sq(model._board[a][b]);
            }
        }

        for (int a = 0; a < _width; a++) {
            for (int b = 0; b < _height; b++) {
                _board[a][b]._successor = model._board[a][b].successor();
                _board[a][b]._predecessor = model._board[a][b].predecessor();
                _board[a][b]._head = model._board[a][b].head();
            }
        }
        _unconnected = model.unconnected();
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq s : this._allSquares) {
            if (s.sequenceNum() != 0 && s.successor() == null) {
                for (Sq qs : this._allSquares) {
                    int qss = qs.sequenceNum();
                    int ss = s.sequenceNum();
                    if (qs != s && qss != 0 && Math.abs(qss - ss) == 1) {
                        if (s.sequenceNum() > qs.sequenceNum()) {
                            result = qs.connect(s);
                        } else {
                            result = s.connect(qs);
                        }
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (seq0 < this.width() * this.height()) {
            int suc = seq0 + 1;
            Place sucP = this.solnNumToPlace(suc);
            return Place.dirOf(x, y, sucP.x, sucP.y);
        }
        return 0;
    }
    ...
    final class Sq {
        ...
        Sq(Sq other) {
            ...
            _head = this;
            _successors = other.successors();
            _predecessors = other.predecessors();
        }
        ...
        void setFixedNum(int n) {
            if (n == 0 || (this.sequenceNum() != 0
                    && this.sequenceNum() != n)) {
                throw badArgs("sequence number may not be fixed");
            }
            this._hasFixedNum = true;
            if (this.sequenceNum() == n) {
                return;
            } else {
                releaseGroup(this.head()._group);
            }
            this._sequenceNum = n;
         ...
         *  nothing. */
        void unfixNum() {
            Sq next = this.successor();
            Sq pred = this.predecessor();
            this._hasFixedNum = false;
            this.disconnect();
            if (pred != null) {
                pred.disconnect();
            }
        ...
        boolean connectable(Sq s1) {
            int s1sn = s1.sequenceNum();
            int tsn = this.sequenceNum();
        ...
        boolean connect(Sq s1) {
            if (this.connectable(s1)) {
                ...
                for (Sq s = this; s.successor() != null; ) {
                    if (s.sequenceNum() != 0 && !s.successor().hasFixedNum()
                            && s.successor().sequenceNum() == 0) {
                        s.successor()._sequenceNum = s.sequenceNum() + 1;
                    }
                    s.successor()._head = this.head();
                    s = s.successor();
                }


                if (s1sn != 0 && tsn == 0) {
                    for (Sq s = s1; s.predecessor() != null; ) {
                        if (!s.predecessor().hasFixedNum()
                                && s.predecessor().sequenceNum() == 0) {
                            s.predecessor()._sequenceNum = s.sequenceNum() - 1;
                        }
                        s = s.predecessor();
                    }
                    releaseGroup(tgroup);
                ...
                if (s1sn == 0 && tsn != 0 && this.hasFixedNum()) {
                    releaseGroup(sgroup);
                }

                if (tsn == 0 && s1sn == 0) {
                    this.head()._group = joinGroups(tgroup, sgroup);
                }
        ...
        void disconnect() {
            ...
            if (!this.hasFixedNum()) {
                ...
                if (this.predecessor() != null
                        && !this.predecessor().hasFixedNum()) {
                    for (Sq s = this.predecessor();
                         s != null && !s.hasFixedNum(); ) {
                        s._sequenceNum = 0;
                        s = s.predecessor();
                    }
            ...
            if (next != null && !next.hasFixedNum()) {
                next._sequenceNum = 0;
                if (next.successor() != null
                        && !next.successor().hasFixedNum()) {
                    for (Sq s = next.successor();
                         s != null && !s.hasFixedNum(); ) {
                        s._sequenceNum = 0;
                        s = s.successor();
                    }
