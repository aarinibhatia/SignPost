...
class PuzzleGenerator implements PuzzleSource {
    ...
    private int makeForwardConnections(Model model) {
        int last = model.width() * model.height();
        int result = 1;
        for (Sq sq : model) {
            Sq found;
            found = null;
            int nFound;
            nFound = 0;
            if (sq.successor() == null && sq.sequenceNum() != last) {
                for (Place curFor : sq.successors()) {
                    Sq s = model.get(curFor);
                    nFound += 1;
                    found = s;
                    if (sq.sequenceNum() != 0 && s.sequenceNum() != 0) {
                        nFound = 1;
                        found = s;
                        break;
                    }
                ...
                if (sq.successors().size() == 0 && sq.sequenceNum() != last && sq.sequenceNum() != 0) {
                    nFound = 0;
                }
                if (nFound == 0) {
                    result = 0;
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place curFor : sq.predecessors()) {
                    Sq s = model.get(curFor);
                    nFound += 1;
                    found = s;
                    if (sq.sequenceNum() != 0 && s.sequenceNum() != 0) {
            ...
            if (sq.predecessors().size() == 0 && sq.sequenceNum() != 1 && sq.sequenceNum() != 1) {
                nFound = 0;
            }
                if (nFound == 0) {
                    return 0;
